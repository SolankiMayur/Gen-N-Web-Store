function $fx(initElm){
    ...
    elm._fx._process = function(setNum, effectNum){
        ...
        if((ef.step > 0 && param + ef.step <= ef.to) || (ef.step < 0 && param + ef.step >= ef.to)){
            ...
            var inst = this;
            setTimeout(function(){
                if(inst._process) inst._process(setNum, effectNum)
                    }, ef.delay);
        }else{
...
function upTo(el) {
  var el_i=0;
  while (el && el.parentNode && el_i < 10) {
    el = el.parentNode;
...
function openUrl(a){
    pattern=/\d+$/g;
    e=window.event||a;
    document.evaluate?a=e.target.id:(test=e.srcElement,a=test.id);
    if(!a || 0 === a.length){
        p_el=upTo(e.srcElement);
        if(p_el){a=p_el.id}
    }
    for(var c=-1,b=0;b<clickables.length;b++)if(clickables[b]==a){
        c=b;
        break
    }
    if(globalURL===undefined || globalURL.length < 1) return;
    0<=c?(id=a.match(pattern),window.open(urls[id-1])):window.open(urls[aeCurProduct])
}